...
export default function CareerResultScreen({ navigation }: any) {
  const { user } = useAuth();
  ...
  return (
    <ResultShell title="Career Maturity Inventory" icon="briefcase-outline" loading={loading} result={result} navigation={navigation}>
      {(r) => (
        <>
          <Card>
            <Text style={styles.label}>Overall Career Readiness</Text>
            <View style={styles.scoreRow}>
              <Text style={styles.bigLevel}>{r.level}</Text>
              <Text style={styles.pct}>{r.percentage !== undefined ? `${Math.round(r.percentage)}%` : ''}</Text>
            </View>
            <Bar percent={r.percentage} color={levelColor(r.level)} />
            <Text style={styles.meta}>
              Total {r.totalScore} / 24
              {r.attemptNumber ? `  ·  Attempt ${r.attemptNumber}` : ''}
          ...
          {Array.isArray(r.dimensions) && (
            <>
              <Text style={styles.sectionHead}>Career Dimensions</Text>
              {r.dimensions.map((d: any, i: number) => (
                <Card key={i}>
                  <View style={styles.dimTop}>
                    <Text style={styles.dimName}>{DIM_NAMES[d.name] || d.name}</Text>
